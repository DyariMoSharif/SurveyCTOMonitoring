...
def Emailing():    
    ...
    try:
        ...
        body = f" ------------------ The ONA Data platform is down ------------------\n \n We cheched the statue of surveycto.com website at {time.localtime().tm_hour}:{time.localtime().tm_min}:{time.localtime().tm_sec} on {time.localtime().tm_mday}/{time.localtime().tm_mon}/{time.localtime().tm_year} (According to local time) \n\n\n ' -- This is an automatic email fired by a python app. For more information, email Dyari at dyari.ameen@gmail.com '\n\n\n"
        em = EmailMessage()
